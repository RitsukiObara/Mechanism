...
CScore* CScore::Create(const D3DXVECTOR3& pos, const D3DXVECTOR3& rot, const D3DXVECTOR3& size)
{
	...
	{ // オブジェクトが NULL じゃない場合
		...
		{
			// 設定処理
			pScore->m_apNumber[nCnt]->SetPos(D3DXVECTOR3(pos.x + (nCnt * 50.0f), pos.y, 0.0f));				// 位置設定
			pScore->m_apNumber[nCnt]->SetRot(rot);				// 向き設定
			pScore->m_apNumber[nCnt]->SetSize(size);			// サイズ設定
